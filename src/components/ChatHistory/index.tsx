...
const ChatMessgeScroll: React.FC<any> = React.forwardRef(({ hideInput = false, msgLists = [], contentList = [], sendMsg }: IProps, ref: any) => {
	...
	// 语音消息
	const ui_audioMessage = (seconds: number, time: string ) => (
  ...
  //   console.log("val", val.avatar)
	...
	// contentList
	const messages = contentList.map(( item: any, index: number ) => {
		item.contentList.map( ( val: any ) => {
			console.log("val", val )
			switch (val.messageType) {
				case 'TEXT':
					return <ChatMessageRow key={index} username={val.sendNickName} sendTime={val.sendTime} headImg={val.avatar} message={ui_audioMessage(val.audioDuration, item.time)} />
				case 'IMG':
					return <ChatMessageRow key={index} username={val.sendNickName} sendTime={val.sendTime} headImg={val.avatar} message={ui_imageMessage(val.content)} />
				case 'TEXT':
					return <ChatMessageRow key={index} username={val.sendNickName} sendTime={val.sendTime} headImg={val.avatar} message={ui_textMessage(val.content)} />
